...
if __name__ == '__main__':
    ...
    count_sql = "select count(*) from JP_jobENTRY; "
    cur.execute(count_sql)
    long_count = cur.fetchone()['count(*)']
    # sql 语句
    big_list = []
    for num in range(1, long_count+1):
        sql = 'select id,Dcontent,LastTime  from JP_jobENTRY where id = %s ' % num
        # #执行sql语句
        cur.execute(sql)
        # #获取所有记录列表
        data = cur.fetchone()
...
# salary,type,link,job_name
    head = ['id', 'Dcontent', "LastTime"]
    csv_dict_write('{0}/JP_jobENTRY.csv'.format(l_path), head, big_list)
    print("数据导出完成～")